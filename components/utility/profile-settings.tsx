...
import { deleteProfile, updateProfile } from "@/db/profile"
...
export const ProfileSettings: FC<ProfileSettingsProps> = ({}) => {
  ...
  const handleClearHistory = async (): Promise<void> => {
    ...
    try {
      // Assuming workspaceId is defined somewhere
      // const workspaceId: string = ""; // Define your workspaceId here

      // Fetch chats by workspace ID
      const chats = await getChatsByWorkspaceId(workspaceId)

      // Delete each chat
      const deletedChats = await Promise.all(
  ...
  const handleDeleteUser = async (): Promise<void> => {
    try {
      ...
      if (data.session) {
        ...
        const deletedUser = await deleteProfile(data.session.user.id) // Assuming deleteProfile function takes user ID as parameter
            ...
            <TabsContent className="mt-4 space-y-4" value="profile">
              ...
              <div className="flex items-center space-y-1">
                <div className="m-1 w-1/2">
                  <Button className="w-full" onClick={handleClearHistory}>
                    Clear history
                  </Button>
                </div>
        ...
        <div className="mt-6 flex items-center">
          <div className="flex items-center space-x-1">
            ...
            <WithTooltip
              display={
                <div>
                  Download Risk Validator 1.0 data as JSON. Import coming soon!
                </div>
              }
              trigger={
                <IconFileDownload
